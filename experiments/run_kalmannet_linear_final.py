...
def main():
    ...
    exp_name = f"knet_linear_d{args.delay}_n{args.noise}_s{args.seed}"

    saver = ResultSaver("results/kalmannet")
    plotter = Plotter("reports/figures/kalmannet")
    ...
    print(f"Delay (residual): {d_res}")
    print(f"Delay (prediction): {d_pred}")

    d_hat = d_pred  # more stable
    ...
    for epoch in range(10):
        with tf.GradientTape() as tape:
            x_hat_seq, y_prior_seq = knet(y_input, sigma2_hat, training=True)
    ...
    rmse = np.sqrt(np.mean((x_hat - y_eval) ** 2))

    print("\nKalmanNet RMSE (vs aligned observation):", rmse)
    ...
    saver.save_history({"knet_loss": knet_losses}, exp_name)
